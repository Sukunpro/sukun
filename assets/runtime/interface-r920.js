...
function make(){
 ...
 <div class="r920MinorTransport"><button id="r920Minus" type="button" aria-label="Bir azalt">−1</button><button id="r920Restart" type="button">Baştan</button><button id="r920Stop" type="button">Bitir</button><button id="r920Plus" type="button" aria-label="Bir artır">+1</button></div>
...
function render(){raf=0;if(!make())return;const s=snap();if(!s)return;
 ...
 $('r920Counter').style.setProperty('--r920-progress',pct);attr($('r920Counter'),'data-near',pct>=90?'1':'0');attr($('r920Counter'),'aria-label',`${name}, ${count} / ${target||'serbest'}. ${journey?'Seyir sayacı':'Bir zikir say'}`);
 const visual=window.SukunR918Visual?.snapshot?.()||{},wheel=visual.wheel||'crystal';
 const src=mode==='berhet'?'./assets/berhetiyye-premium/'+({crystal:'wheel-alpha-r915.png',seal:'wheel-seal-r916.svg',pearls:'wheel-pearls-r916.svg'}[wheel]||'wheel-alpha-r915.png'):'./assets/sukun-nur-ring-r757.png';
 if($('r920Wheel').getAttribute('src')!==src)$('r920Wheel').src=src;
