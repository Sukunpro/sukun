...
module.exports={fixture,finalRule,style};
if(require.main===module)(async()=>{
 ...
 await test('Hûtîrin automatically loads a packaged Süleyman scene and preserves count',async()=>{
  const f=fixture();f.unlock(true);f.Z.cat='berhet';f.Z.idx=9;f.load('sukun-r831-hard-fix-runtime');await f.image('scene-10-hutirin');
  assert.equal(f.document.documentElement.dataset.r835Theme,'berhet');assert.equal(f.document.documentElement.dataset.r832Scene,'hutirin');assert.equal(f.Z.count,19);
 });
 await test('All twelve explicit backgrounds resolve; invalid choice returns to auto',async()=>{
  const f=fixture();f.unlock(true);f.Z.cat='berhet';f.load('sukun-r831-hard-fix-runtime');
  const a=f.c.SukunR832VisualAuthority;assert.equal(a.scenes.length,13);
 ...
 await test('Slow earlier scene cannot overwrite a newer selection',async()=>{
  ...
  a.setScene('water');a.setScene('night');await f.image('scene-07');assert.equal(a.snapshot().scene,'night');await f.image('scene-03');await f.image('scene-09-berhetihin');assert.equal(a.snapshot().scene,'night');
 });
 await test('Relock and 99 Esma transition cancel pending premium paints',async()=>{
  const f=fixture();f.unlock(true);f.Z.cat='berhet';f.load('sukun-r831-hard-fix-runtime');const a=f.c.SukunR832VisualAuthority;
  f.Z.cat='esma';a.sync();await f.image('scene-09-berhetihin');assert.equal(a.snapshot().bgmode,'generic');assert(!f.document.documentElement.style.getPropertyValue('--r832-berhet-bg'));
 ...
 await test('Counter notifications do not issue repeated image downloads',async()=>{
  const f=fixture();f.unlock(true);f.Z.cat='berhet';f.load('sukun-r831-hard-fix-runtime');await f.image('scene-09-berhetihin');const before=f.pending.length;
 ...
 await test('Chooser contains every mapped scene and changes through its real control',async()=>{
  ...
  assert.equal(s.options.length,13);assert.equal(f.$('r829BerhetScenes').children.length,13);s.value='seal';s.dispatchEvent(new f.dom.Event('change',{bubbles:true}));await f.image('scene-06');assert.equal(a.snapshot().scene,'seal');
